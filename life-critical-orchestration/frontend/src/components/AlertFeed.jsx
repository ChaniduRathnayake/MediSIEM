// Left-column list of all available stub alerts. Clicking one fires the
// onSelect callback, which the parent uses to call POST /decide.
...
export default function AlertFeed({ alerts, selectedId, onSelect, busy }) {
  ...
  const filtered = useMemo(() => {
    ...
    return alerts.filter((a) => {
      const haystack = [
        ...
        a.threat?.category,
        // Live alerts carry their classification on _liveDecision; including
        // decision_id here lets analysts paste an audit-trail UUID into the
        // search box and jump straight to the originating alert.
  ...
  return (
    <>
      ...
      <ul className="divide-y divide-soc-border">
        {filtered.length === 0 && (
          <li className="px-4 py-3 text-soc-muted text-xs">
            No alerts match.
          </li>
        )}
        {filtered.map((alert) => {
          const isSelected = alert.alert_id === selectedId;
          const accent = tierColor[alert._expectedTier] || "border-l-soc-muted";
            ...
            <li key={alert.alert_id}>
              ...
              >
                <div className="flex items-baseline justify-between gap-2 mb-1">
                  <span className="text-soc-text text-sm font-bold truncate">
