...
import IpReputationCache from '../models/IpReputationCache.js';
...
// (likely repeat-offender) source IP. Persisted in MongoDB (IpReputationCache,
// TTL-indexed on checkedAt) so the cache survives process restarts.
...
export async function checkIpReputation(ip) {
  ...
  const cached = await IpReputationCache.findOne({ ip }).lean();
  ...
  try {
    ...
    await IpReputationCache.findOneAndUpdate(
      { ip },
      { ip, score, checkedAt: new Date() },
      { upsert: true }
    );
    return score;
  } catch (err) {
