...
const ComplianceDetailModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-3xl rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden max-h-[88vh] flex flex-col"
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-4 gap-3">
                <div className="rounded-xl bg-slate-800/60 border border-slate-800 p-3 text-center">
                ...
                <div className="rounded-xl bg-slate-800/60 border border-slate-800 p-3 text-center">
                  <p className="text-lg font-bold text-slate-400">{naChecks.length}</p>
                ...
                <>
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-8 pr-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/60 w-48"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 max-h-[45vh] overflow-y-auto pr-1">
                    {filteredCisChecks.length === 0 ? (
                      <p className="text-xs text-slate-500 text-center py-8">No checks match.</p>
                    ) : (
                      filteredCisChecks.map((c, i) => (
                        <div key={c.id ?? i} className={`rounded-lg border p-3 ${checkCardClass(c)}`}>
                          ...
                          {c.description && <p className="text-xs text-slate-400 mt-1.5">{c.description}</p>}
                          {c.rationale && (
                            <p className="text-xs text-slate-500 mt-1"><span className="text-slate-400 font-medium">Rationale: </span>{c.rationale}</p>
                          )}
                          {c.remediation && (
                            <p className="text-xs text-slate-500 mt-1"><span className="text-slate-400 font-medium">Remediation: </span>{c.remediation}</p>
                          )}
                        </div>
                      ))
                    )}
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-slate-800/60 border border-slate-800 p-3 text-center">
                  ...
                  <p className="text-xs text-slate-500">Controls flagged</p>
                </div>
                <div className="rounded-xl bg-slate-800/60 border border-slate-800 p-3 text-center">
              ...
              {controlView === 'flagged' ? (
                          ...
                          <div className="mt-2 pt-2 border-t border-red-500/10 space-y-1">
                            {v.samples.map((s, i) => (
                              <p key={i} className="text-xs text-slate-500">
                                {s.timestamp && <span className="text-slate-600 font-mono">{new Date(s.timestamp).toLocaleString()}</span>}
                                {s.description && <span> — "{s.description}"</span>}
                                {s.ruleId && <span className="text-slate-600"> (rule {s.ruleId})</span>}
                              </p>
                            ))}
                          </div>
                        )}
                ...
                <div className="flex flex-wrap gap-1.5 max-h-[45vh] overflow-y-auto pr-1">
                  {filteredCompliant.length === 0 ? (
                    <p className="text-xs text-slate-500 text-center py-8 w-full">No controls match.</p>
                  ) : (
              ...
              <div className="flex items-start gap-2 p-3 rounded-lg bg-slate-800/40 border border-slate-800">
    ...
    return (
      <div className="p-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">{FRAMEWORK_META[framework].description}</p>
        </div>
        <div className="mt-5 rounded-2xl bg-slate-900 border border-slate-800 p-10 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1">Connect it under Settings → Wazuh SIEM to see compliance data here.</p>
        </div>
      </div>
    );
  }
            ...
            {[7, 30, 90].map((d) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  days === d ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30' : 'bg-slate-800 text-slate-400 border border-slate-700 hover:text-white'
                }`}
              >
                {d}d
              </button>
                  ...
                  return (
                    <tr key={ag.id} className="border-b border-slate-800/60 last:border-0 hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3 px-5 text-sm">
                        <button onClick={() => setSelectedAgent(ag)} className="text-white font-medium hover:text-cyan-400 transition-colors text-left">
                          {ag.name}
                        </button>
                      </td>
                        ...
                        {framework === 'cis' ? (
                          sca && sca.ok && sca.total !== undefined && sca.total > 0 ? (
                            <span className="flex items-center gap-2">
                              ...
                              <span className="text-slate-500">{sca.pass} pass / {sca.fail} fail</span>
                            </span>
                          ) : (
                            <span className="text-slate-600">No CIS scan data</span>
                          )
                        ) : violated === 0 ? (
